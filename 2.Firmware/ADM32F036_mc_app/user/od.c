...
static const OD_entry_t ODList[] = 
{
    ...
    {0x2070, &ODObjs.in_encoder_offset,         2, ATTR_RAM | ATTR_RW, enc_set_zero}, // 标零只需要写入一次0x2070即可
...
uint16_t OD_write_2(uint16_t idx, uint16_t *data)
{
    ...
    // 标零安全检查：电机旋转有速度时拒绝标零，防止关中断擦写 Flash 导致 FOC 失控和电源异常
    if(idx == 0x2070)
    {
