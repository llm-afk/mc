...
void MC_servo_loop(void)
{
    ...
    switch(motor_ctrl.state)
    {
        ...
        case MIT:
        {
            ...
            // =========================================================================
            // 【FIX】故障阻尼保护：固定 Kd + 固定 5 秒后停机
            // =========================================================================
            #define FAULT_DAMP_TICKS   10000          // @ 2kHz = 5 秒
            #define FAULT_DAMP_KD_Q14  (16400u * 1u) 

            if(ODObjs.error_code != 0 || fault_damp_timer > 0)
            {
                // Kd-only 阻尼：目标速度 = 0，使用固定 Kd 不依赖 CAN 下发值
            ...
            {
                fault_damp_timer = 0;  // 无故障且正常运行，保持复位
            }
            break;
        }
...
void info_collect_loop(void)
{
    ...
    else
    {
        ...
        {
            // 只要没到100，就正常累加；到了100就锁死，等待CAN接收中断将其清零
            ...
            {
                // 如果 cnt < 100，说明外部接收中断刚刚清零过它，通信已经恢复！
